 ...
 * Uses rtf-stream-parser as the primary parsing method with manual fallback
 */
...
function preprocessRTFContent(content: string): string {
  ...
  processed = processed.replace(/(\d{1,2}[A-Z]{3})(ACFTTYPE:|ACFTREG:|HEADERVERSION:|ROUTE:|STD:|PREPAREDBY:|TTLPLNULD:|ULDVERSION:|PREPAREDON:|SECTOR:)/gi, (match, dateMonth, field) => {
...
export function parseRTFHeader(content: string): LoadPlanHeader {
  ...
  // Parse Route - try multiple patterns
  // Example: "Route: DXB/LHR/13/75/320" - can have slashes, letters, numbers
  const pax = tryPattern('Route', [
    { pattern: /ROUTE:\s*([A-Z0-9\/\s]+?)(?:\s{2,}STD|\s+PREPARED|$)/i, extract: (m) => m[1].trim() }, // Stop at multiple spaces before STD
    { pattern: /ROUTE:\s*([A-Z0-9\/\s]+?)(?=\s+STD|\s+PREPARED|$)/i, extract: (m) => m[1].trim() }, // Lookahead for STD or PREPARED
    { pattern: /ROUTE[:\s]+([A-Z0-9\/]+)/i, extract: (m) => m[1].trim() },
  ])
